DROP TABLE PSH_TEST01;

CREATE TABLE PSH_TEST01 AS
WITH TEMP01 AS (
-------------------
-- 스케줄 수행대상건 조회
-------------------
...
SELECT A.SCHEDULE_ID, A.SCENARIO_ID, A.TYPE_CD, A.RESULT_TIME
     --, A.BIN_X_CNT, A.BIN_Y_CNT
FROM   SCHEDULE A
WHERE  1=1
AND    A.TYPE_CD = 'SC001'
...
SELECT
     ...
     , ROUND(A.BIN_CNT * NVL(B.RU_CNT,0)) AS JOB_THRESHOLD
     , ROUND(A.RESULT_TIME) AS RESULT_TIME
FROM   TEMP06 A
     , TEMP07 B
WHERE  A.SCENARIO_ID = B.SCENARIO_ID(+)
;

SELECT * FROM PSH_TEST01
WHERE RESULT_TIME > 600000
;
