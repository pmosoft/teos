...
FROM  (
		   ...
		   FROM   JOB_DIS A
		   WHERE  A.SCENARIO_ID IN (SELECT CAST(SCENARIO_ID AS TEXT) FROM I_SCHEDULE WHERE SCHEDULE_ID = ${scheduleId})
       ) A,
      (		
...
WHERE  SCENARIO_ID IN (SELECT CAST(SCENARIO_ID AS TEXT) FROM I_SCHEDULE WHERE SCHEDULE_ID = ${scheduleId})
AND    STAT = 3
"""
}  
