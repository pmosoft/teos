...
def selectRuCnt(scheduleId:String,typeCd:String,bdYn:String) = {
s"""
...
FROM  (
		   SELECT COUNT(A.RU_ID)                                 AS RU_CNT
		        , COUNT(CASE WHEN '${typeCd}' <> 'SC051' AND '${bdYn}' = 'N' AND A.STAT=3 THEN 1 
                         WHEN '${typeCd}' <> 'SC051' AND '${bdYn}' = 'Y' AND A.STAT=4 THEN 1
                         WHEN '${typeCd}'  = 'SC051' AND '${bdYn}' = 'Y' AND A.STAT=4 THEN 1
                         ELSE NULL END) AS POST_DONE_CNT
		   FROM   JOB_DIS A
...
def selectExtRu(scheduleId:String,typeCd:String,bdYn:String) = {
s"""
SELECT A.RU_ID
     , A.CLUSTER_NAME	     
FROM  (SELECT * 
       ...
       AND    STAT = CASE WHEN '${typeCd}' <> 'SC051' AND '${bdYn}' = 'N' THEN 3 
                          WHEN '${typeCd}' <> 'SC051' AND '${bdYn}' = 'Y' THEN 4
                          WHEN '${typeCd}'  = 'SC051' AND '${bdYn}' = 'Y' THEN 4
                          ELSE NULL END
       ) A
       LEFT OUTER JOIN (
...
def deleteJobDisExt(scheduleId:String) = {
s"""
DELETE FROM JOB_DIS_ETL
WHERE  SCHEDULE_ID = '${scheduleId}'
"""
}
