...
FROM   RU_ANTENA  ANTENA
     , ANTENABASE BASE
     , RU         TRU
WHERE  ANTENA.SCENARIO_ID IN (SELECT SCENARIO_ID FROM SCHEDULE WHERE SCHEDULE_ID = ${scheduleId})
AND    TRU.SCENARIO_ID    IN (SELECT SCENARIO_ID FROM SCHEDULE WHERE SCHEDULE_ID = ${scheduleId})
