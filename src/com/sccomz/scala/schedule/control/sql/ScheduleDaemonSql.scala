...
-- 스케줄 수행대상건 조회
-------------------
SELECT A.SCHEDULE_ID, A.SCENARIO_ID, A.TYPE_CD
...
SELECT
       A.SCHEDULE_ID
     , SUM((B.BD_FLOOR_MAX * B.BD_AREA) / POWER(B.BUILDINGANALYSIS3D_RESOLUTION,2) AS BD_AREA
     --, ADDR_ADM_DONG
     --, BD_FLOOR_MAX
     --, BD_AREA
     --, WTM_SX
...
FROM   TEMP02  A
     , BD_LOSS B
WHERE  A.TYPE_CD IN (SELECT TYPE_CD FROM SCHEDULE_TYPE_CD WHERE USE_YN = 'Y' AND BD_YN = 'Y')
AND    B.WTM_EX >= TM_STARTX
AND    B.WTM_SX <= TM_ENDX
AND    B.WTM_EY >= TM_STARTY
AND    B.WTM_SY <= TM_ENDX
...
SELECT
     ...
     , A.AREA
     , NVL(B.RU_CNT,0)             AS RU_CNT
     , A.BIN_CNT * NVL(B.RU_CNT,0) AS JOB_THRESHOLD
FROM   TEMP06 A
     , TEMP07 B
     , TEMP08 C
WHERE  A.SCENARIO_ID = B.SCENARIO_ID(+)
